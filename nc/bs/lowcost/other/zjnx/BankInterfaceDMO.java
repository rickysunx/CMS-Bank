 ...
 * @author 孙锐
 */
package nc.bs.lowcost.other.zjnx;
...
//    	}
        ...
        if(newSum.compareTo(new UFDouble(0.0f))>0){
            (new VoucherDealDMO()).doLowCostTradeByCorp(acc,pk_corp,String.valueOf(date.getYear()),date.getStrMonth(),"1",newSum);
        }
        if(cancelSum.compareTo(new UFDouble(0.0f))>0){
            (new VoucherDealDMO()).doLowCostTradeByCorp(acc,pk_corp,String.valueOf(date.getYear()),date.getStrMonth(),"2",cancelSum);
        }
    }
