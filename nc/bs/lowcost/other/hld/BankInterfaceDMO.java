 ...
 * @author 孙锐
 */
package nc.bs.lowcost.other.hld;
...
import nc.bs.hld.expvoucher.VoucherDealDMO;
...
//    	}
        ...
        if(newSum.compareTo(new UFDouble(0.0f))>0){
            System.out.println("=====调用葫芦岛接口====,新增卡片");
            (new VoucherDealDMO()).doLowCostTrade(pk_corp,acc,"01","2",newSum);
        }
        if(cancelSum.compareTo(new UFDouble(0.0f))>0){
            System.out.println("=====调用葫芦岛接口====,核销卡片");
            (new VoucherDealDMO()).doLowCostTrade(pk_corp,acc,"01","1",cancelSum);
        }
        
        System.out.println("=====调用葫芦岛接口====,完成");
    }
