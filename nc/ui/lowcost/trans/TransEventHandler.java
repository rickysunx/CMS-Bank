 ...
 *
 */
public class TransEventHandler extends ManageEventHandler {

    protected void buttonActionAfter(AbstractBillUI billUI, int intBtn) throws Exception {
     ...
     * @param control
     */
    public TransEventHandler(BillManageUI billUI, IControllerBase control) {
    ...
    protected UIDialog getQueryUI() throws Exception {
        ...
        return new TransQueryDlg(getBillUI());
    }
    ...
    protected void onBoEdit() throws Exception {
        ...
        if(pk_maker.equals(operator)){
        	super.onBoEdit();
        	getBillCardPanelWrapper().getBillCardPanel().getHeadItem("transcode").setEnabled(false);
        }else{
    ...
    protected IBusinessController createBusinessAction() {
    	...
    	TransBusinessAction temp = new TransBusinessAction(getBillUI());
        return temp;
    ...
    protected void onBoQuery() throws Exception {
		...
		if (querydialog.showModal() == UIDialog.ID_OK) {
			...
			strWhere += " order by lc_trans.transcode";
        ...
        if(bo.getName().equals("批量增加")){
            ...
            if(pk_lccards!=null && pk_lccards.length>0){
                for (int i = 0; i < pk_lccards.length; i++) {
                    ...
                    cp.getBillTable().editCellAt(n,2);
                }
            }
        }
