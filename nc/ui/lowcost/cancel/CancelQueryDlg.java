...
import nc.ui.lowcost.trans.UserRefModel;
 ...
 *
 */
public class CancelQueryDlg extends UIDialog implements INormalQuery {

	/**
	 * 
	 */
	private static final long serialVersionUID = 9143027093246294317L;
     ...
     * @param parent
     */
    public CancelQueryDlg(Container parent) {
        super(parent);
		initialize();
    }
     ...
     * @param title
     */
    public CancelQueryDlg(Container parent, String title) {
        super(parent, title);
		initialize();
    }
     ...
     * @param owner
     */
    public CancelQueryDlg(Frame owner) {
        super(owner);
		initialize();
    }
     ...
     * @param title
     */
    public CancelQueryDlg(Frame owner, String title) {
        super(owner, title);
		initialize();
    }
    ...
    public String getWhereSql() {
        ...
        if(tallyStatus!=null && (!tallyStatus.equals(""))){
            if(tallyStatus.equals("未记账")){
                whereSql += " and (tallyflag = 'N' or tallyflag is null) ";
            }
        ...
        try {
            ...
            whereSql += " and pk_cancel in (select pk_cancel from lc_cancel where pk_cancel in (select pk_cancel from lc_cancel_m where pk_lccard in (select pk_lccard from lc_card where pk_usedept in (select pk_deptdoc from bd_deptdoc where pk_corp = '" + pk_corp +  "'))))";
        } catch (Exception e){
            e.printStackTrace();
        }
        return whereSql;
	...
	private UIButton getBtnCancel() {
		if (btnCancel == null) {
			...
			btnCancel.setBounds(275, 175, 93, 22);
